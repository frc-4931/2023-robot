...
public class Drivetrain extends SubsystemBase {
  public enum Multiplier {
    FULL(1d), THREE_QUARTERS(0.75), HALF(0.5);
    ...
    public Multiplier next() {
      return values()[(this.ordinal() + 1) % 3];
    }

    public Multiplier previous() {
      return values()[Math.abs((this.ordinal() -1) % 3)];
    }
  }
  ...
  public void drive(double xSpeed, double ySpeed, double zSpeed) {
    ...
    var times = multiplier.getValue();
    drive.driveCartesian(xSpeed*times, ySpeed*times, zSpeed, z);
  }
  ...
  public Command wheelSpeedDriveCommand(DoubleSupplier xSupplier, DoubleSupplier ySupplier, 
    ...
    return this.run(() -> {
      var wheelSpeeds = DRIVE_KINEMATICS.toWheelSpeeds(
        fieldOriented 
          ? ChassisSpeeds.fromFieldRelativeSpeeds(xSupplier.getAsDouble(), ySupplier.getAsDouble(), zSupplier.getAsDouble(), getAngle()) 
  ...
  public Command lowerMultiplier() {
    return this.runOnce(() -> multiplier = multiplier.next());
  }

  public Command raiseMultiplier() {
    return this.runOnce(() -> multiplier = multiplier.previous());
  }
